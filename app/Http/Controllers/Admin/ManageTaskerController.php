...
class ManageTaskerController extends Controller
{
    public function index()
    {
        ...
        $user = Auth::user();
        return view('pages.admin.manage.tasker', compact('user', 'tasker'));
    }

    public function getTasker(Request $request)
    {
        if ($request->ajax()) {
            $data = User::where('role', 'tasker')->latest();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    return '
                        <button class="btn bg-brown text-white" onclick="editTasker(' . $row->id . ')">
                            <i class="fa-regular fa-pen-to-square"></i>
                        </button>
                        <button class="btn bg-brown text-white" onclick="deleteTasker(' . $row->id . ')">
